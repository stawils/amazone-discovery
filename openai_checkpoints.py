...
class OpenAIIntegration:
    ...
    def analyze_with_openai(self, prompt: str, data_context: str = "", model: str = "gpt-4") -> Dict[str, Any]:
        ...
        try:
            response = self.client.chat.completions.create(
                model=model,
                messages=[
                    {"role": "system", "content": "You are an expert archaeologist specializing in Amazon pre-Columbian civilizations and remote sensing."},
...
class CheckpointRunner:
    ...
    def run(self, checkpoint_num: int, **kwargs):
        ...
        try:
            ...
            result_file = self.checkpoint_dir / f"checkpoint_{checkpoint_num}_result.json"
    ...
    def checkpoint2_early_explorer(self, zones: List[str] = None, max_scenes: int = 2) -> Dict[str, Any]:
        ...
        try:
            ...
            for scene in all_scenes[:4]:  # Process top 4 scenes
                ...
                try:
                    ...
                    if analysis_result.get('success'):
                        ...
                        tp_patches = analysis_result.get('terra_preta', {}).get('patches', [])
                        ...
                        for feature in geom_features:
                            if feature.get('center') or feature.get('pixel_center'):
                                coords = feature.get('center') or feature.get('pixel_center')
                                anomaly_footprints.append({
                                    'type': f"geometric_{feature.get('type', 'unknown')}",
                                    'coordinates': coords,
                                    'confidence': feature.get('confidence', 0),
                                    'size_m': feature.get('diameter_m') or feature.get('length_m', 0),
                                    'scene_id': scene.scene_id,
                                    'zone': scene.zone_id
                                })
                        ...
                        openai_analysis = self.openai_integration.analyze_with_openai(
                            prompt, 
                            f"Scene: {scene.scene_id}, Zone: {zone.name}"
                        )
                ...
                if scene_dir.exists():
                    ...
                    if analysis_result2.get('success'):
                        result['reproducibility_test'] = {
                            'scene_id': scene.scene_id,
                            'run1_features': len(analysis_result.get('geometric_features', [])),
                            'run2_features': len(analysis_result2.get('geometric_features', [])),
                            'consistent': abs(len(analysis_result.get('geometric_features', [])) - 
                                           len(analysis_result2.get('geometric_features', []))) <= 1
                        }
            ...
            for i, footprint in enumerate(top_5_footprints):
                ...
                print(f"  Anomaly {i+1}: {footprint['type']} at {coords[1]:.4f}, {coords[0]:.4f} "
                      f"(confidence: {footprint['confidence']:.2f})")
            
            result['success'] = True
            result['summary'] = f"Found {len(anomaly_footprints)} anomalies across {len(all_scenes)} scenes"
    ...
    def checkpoint3_site_discovery(self, zone: str = 'negro_madeira') -> Dict[str, Any]:
        ...
        try:
            ...
            result['best_discovery'] = {
                ...
                'properties': {k: v for k, v in best_feature.items() 
            ...
            comparison_analysis = self.openai_integration.analyze_with_openai(
                comparison_prompt,
                f"New discovery at {zone_info.name}"
            )
            ...
            result['success'] = True
            result['summary'] = f"Discovered {best_feature['discovery_type']} with {best_feature.get('confidence', 0):.2f} confidence"
    ...
    def checkpoint4_story_impact(self, zone: str = 'negro_madeira') -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self, 'checkpoint_results') and 'checkpoint_3' in self.checkpoint_results:
                discovery_data = self.checkpoint_results['checkpoint_3'].get('best_discovery', {})
            else:
                # Run checkpoint 3 to get discovery data
                discovery_data = self.checkpoint3_site_discovery(zone).get('best_discovery', {})
            ...
            story_analysis = self.openai_integration.analyze_with_openai(
                story_prompt,
                f"Archaeological discovery at {zone_info.name}"
            )
            ...
            hypothesis_analysis = self.openai_integration.analyze_with_openai(
                hypothesis_prompt,
                f"Function and age analysis for {zone_info.name}"
            )
            ...
            survey_analysis = self.openai_integration.analyze_with_openai(
                survey_prompt,
                f"Survey proposal for {zone_info.name}"
            )
            ...
            with open(doc_path, 'w') as f:
                ...
                f.write(f"**Discovery Summary:** {story_document['executive_summary']}\n\n")
                f.write(f"**Coordinates:** {story_document['discovery_coordinates']}\n\n")
            ...
            print(f"Narrative Length: ~{len(story_analysis.get('response', '').split())} words")
            print(f"Presentation Slides: {len(presentation_outline)} planned")
            
            result['success'] = True
            result['summary'] = f"Created comprehensive story and impact draft for {zone_info.name}"
    ...
    def checkpoint5_final_submission(self) -> Dict[str, Any]:
        ...
        try:
            ...
            final_package['discovery_statistics'] = {
                ...
                'zones_analyzed': len(set(d.get('zone', '') for d in discoveries_summary if 'zone' in d)),
            ...
            evidence_assessment = {
                ...
                'ai_enhancement': 'Innovative - First use of GPT-4 for archaeological pattern interpretation'
            }
            ...
            submission_doc = {
                ...
                """,
                'archaeological_impact': final_assessment.get('response', 'Assessment pending')
            }
            ...
            presentation_summary = self.checkpoint_dir / "livestream_presentation_summary.md"
            with open(presentation_summary, 'w') as f:
                ...
                f.write(f"- **{total_anomalies} archaeological anomalies** detected across Amazon basin\n")
                f.write(f"- **{high_confidence_sites} high-confidence sites** requiring ground verification\n")
                f.write(f"- **{len(TARGET_ZONES)} priority zones** systematically analyzed\n")
                f.write("- **First convergent anomaly detection** system for archaeology\n")
                f.write("- **OpenAI integration** for pattern interpretation and validation\n\n")
            ...
            print(f"Detection Success Rate: {final_package['discovery_statistics']['detection_success_rate']}")
            ...
            result['success'] = True
            result['summary'] = f"Final submission completed with {total_anomalies} discoveries ready for livestream"
    ...
    for i in range(1, 6):
        try:
            ...
            if result.get('success'):
                ...
                print(f"❌ Checkpoint {i} failed: {result.get('error', 'Unknown error')}")
