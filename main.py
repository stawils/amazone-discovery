...
def run_modular_pipeline(**kwargs):
    ...
    try:
        ...
        if results.get("report"):
            session_id = (
                results["report"].get("session_info", {}).get("session_id", "unknown")
            )
...
Examples:
  ...
  python main.py --competition-ready
    ...
    parser.add_argument(
        "--provider",
        choices=["gee", "sentinel2", "gedi"],
        default="gee",
        help="Data provider (gee, sentinel2, or gedi)",
    )

    # Pipeline options
    parser.add_argument(
        "--max-scenes", type=int, default=3, help="Maximum scenes per zone (default: 3)"
    )
    parser.add_argument(
