...
class ArchaeologicalVisualizer:
    ...
    def _add_discoveries_with_cluster(
        self, map_obj: folium.Map, analysis_results: Dict, scoring_results: Dict = None
    ):
                        ...
                        geom_data.append(
                            {
                                ...
                                "icon": folium.Icon(
                                    color="blue", icon="minus", prefix="fa"
                                ),
                            }
                        )
                        ...
                        geom_data.append(
                            {
                                ...
                                "icon": folium.Icon(
                                    color="purple", icon="square", prefix="fa"
                                ),
                            }
                        )
    ...
    def _add_geometric_feature_marker(
        self, group: plugins.FastMarkerCluster, feature: Dict, zone_name: str
    ):
    ...
    def add_gedi_discoveries(self, map_obj: folium.Map, gedi_results: Dict):
        """Add GEDI discovery markers to map."""
        gedi_group = folium.FeatureGroup(name="GEDI Space LiDAR")

        for gap in gedi_results.get("canopy_gaps", []):
            folium.CircleMarker(
                location=[gap["lat"], gap["lon"]],
                radius=8,
                popup=f"Canopy Gap: {gap['height']:.1f}m",
                tooltip="Potential Archaeological Clearing",
                color="lightgreen",
                fillColor="lightgreen",
                fillOpacity=0.7,
            ).add_to(gedi_group)

        for anomaly in gedi_results.get("elevation_anomalies", []):
            folium.CircleMarker(
                location=[anomaly["lat"], anomaly["lon"]],
                radius=10,
                popup=f"Elevation Anomaly: {anomaly['elevation']:.1f}m",
                tooltip="Potential Earthwork/Mound",
                color="purple",
                fillColor="purple",
                fillOpacity=0.8,
            ).add_to(gedi_group)

        gedi_group.add_to(map_obj)
    ...
    def create_scoring_dashboard(
        self, scoring_results: Dict[str, Dict], output_path: Path = None
    ) -> Path:
            ...
            go.Bar(
                ...
                marker_color=[
                    (
                        "red"
                        if s >= 10
                        else "orange" if s >= 7 else "yellow" if s >= 4 else "gray"
                    )
                    for s in scores
                ],
            ),
        ...
        fig.add_trace(
            go.Histogram(
                x=scores, nbinsx=10, name="Score Distribution", marker_color="lightblue"
            ),
            row=1,
            col=2,
        )
    ...
    def create_feature_analysis_plots(
        self, analysis_results: Dict, output_dir: Path = None
    ) -> List[Path]:
    ...
    def _create_terra_preta_plots(
        self, analysis_results: Dict, output_dir: Path
    ) -> Path:
        ...
        for zone in df["zone"].unique():
            ...
            axes[0, 1].scatter(
                zone_data["tp_index"], zone_data["confidence"], label=zone, alpha=0.7
            )
        ...
        scatter = axes[1, 1].scatter(
            df["area_ha"], df["confidence"], c=df["tp_index"], cmap="viridis", alpha=0.7
        )
    ...
    def _create_geometric_plots(self, analysis_results: Dict, output_dir: Path) -> Path:
        ...
        df_pivot = df.pivot_table(
            values="confidence", index="zone", columns="type", aggfunc="mean"
        )
        ...
        for ftype in df["type"].unique():
            type_data = df[df["type"] == ftype]
            axes[1, 1].scatter(
                type_data["size"], type_data["confidence"], label=ftype, alpha=0.7
            )
    ...
    def _create_zone_comparison(self, analysis_results: Dict, output_dir: Path) -> Path:
            ...
            zone_data.append(
                {
                    ...
                    "scenes_analyzed": len(
                        [s for s in zone_results if s.get("success")]
                    ),
                }
            )
        ...
        bars = axes[0, 0].bar(
            ...
            df["total_features"],
            color=[
                "red" if p == 1 else "orange" if p == 2 else "blue"
        ...
        x = np.arange(len(df))
        axes[0, 1].bar(
            x - width / 2, df["tp_patches"], width, label="Terra Preta", alpha=0.8
        )
        axes[0, 1].bar(
            x + width / 2, df["geometric_features"], width, label="Geometric", alpha=0.8
        )
...
if __name__ == "__main__":
    ...
    map_path = visualizer.create_discovery_map(
        mock_analysis, mock_scoring, Path("test_map.html")
    )
    print(f"✓ Test map created: {map_path}")

    # Test dashboard
    dashboard_path = visualizer.create_scoring_dashboard(
        mock_scoring, Path("test_dashboard.html")
    )
